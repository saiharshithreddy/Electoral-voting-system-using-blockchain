...
contract VoterContract   {
    ...
    mapping(uint => uint256) public votedTime;
    ...
    function userVoted(uint _user_id) public view returns (uint256){
        return votedTime[_user_id];
    }

    function vote (uint _candidateId, uint _user_id) public {
        if(addressVote[msg.sender]!=1)
        {
            addressVote[msg.sender] = 1;
            candidates[_candidateId].voteCount++;
            voterVoted[_user_id] = true;
            votedTime[_user_id] = now;
        }
    }
